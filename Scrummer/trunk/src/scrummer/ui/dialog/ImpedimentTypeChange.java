...
/**
 * Dialog for modifying impediment type
 */
public class ImpedimentTypeChange
	extends TwoButtonDialog
	implements ImpedimentListener {

	public ImpedimentTypeChange(JFrame parent) {
		super(parent);
		
		setTitle(i18n.tr("Change Impediment Type"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals("StandardDialog.OK")) {
			if (Validate.empty(_impedimentDescriptionInput, this)) {
				_impedimentModel.changeType(
					_impedimentTypeInput.getSelectedId(), 
	...
	public void operationSucceeded(DataOperation type, ImpedimentOperation identifier, String message) {
		if ((type == DataOperation.Update) && (identifier == ImpedimentOperation.ImpedimentType)) {
			setVisible(false);
		}	
	}
	...
	public void operationFailed(DataOperation type, ImpedimentOperation identifier, String message) {
		if ((type == DataOperation.Update) && (identifier == ImpedimentOperation.ImpedimentType)) {
			Util.showError(
				this, i18n.tr("Could not update impediment status: " + message), 
				i18n.tr("Error"));
		}
	}
	...
	/// serialization id
	private static final long serialVersionUID = 2380102405156625261L;
}
